...
import { homeCoursesStyles } from "../assets/styles/dummyStyles";
import { coursesData } from "../data/dummyHdata";
...
export const HomeCourses = () => {
  ...
  const handleBrowseClick = () => {
    const token = localStorage.getItem("token");
    if (!token) {
      toast("Please login to access courses", {
        position: "top-right",
        transition: Slide,
        autoClose: 3000,
        theme: "dark",
              ...
              >
                <div className={homeCoursesStyles.imageContainer}>
                  <img
                    src={course.image}
                    alt={course.name}
                    className={homeCoursesStyles.image}
                    loading="lazy"
                  />
                </div>
